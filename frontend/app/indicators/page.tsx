...
import { CheckCircleIcon, CircleStackIcon, PlusIcon, XCircleIcon } from "@heroicons/react/24/outline";
import { FormEvent, useCallback, useEffect, useState } from "react";

import { AppShell } from "@/components/app-shell";
import { getToken } from "@/lib/auth";
import { createIndicator, getIndicators, updateIndicator } from "@/services/api";
      ...
      <div className="p-6 lg:p-10">
        ...
        <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
          <form onSubmit={submit} className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="flex items-center gap-2 font-bold text-slate-800"><PlusIcon className="h-5 w-5 text-cyan-700" />Tambah Indikator</h2>
            ...
            <button disabled={saving} className="mt-5 w-full rounded-xl bg-[#102a43] px-4 py-3 font-semibold text-white disabled:opacity-50">{saving ? "Menyimpan..." : "Simpan Indikator"}</button>
          </form>
