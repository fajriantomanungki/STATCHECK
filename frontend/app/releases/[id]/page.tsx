...
import { ArrowLeftIcon, ChatBubbleLeftRightIcon, CheckCircleIcon, ClockIcon, DocumentTextIcon, PlayCircleIcon, PlusIcon, TrashIcon, UserGroupIcon } from "@heroicons/react/24/outline";
...
export default function ReleaseDetailPage() {
  ...
  return <AppShell title="Detail Kegiatan Rilis"><div className="p-6 lg:p-10">
    ...
    {!release ? <p className="mt-10 text-sm text-slate-400">Memuat kegiatan rilis...</p> : <>
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-3"><Link href={`/releases/${id}/guests`} className="rounded-2xl border border-slate-200 bg-white p-6 transition hover:border-cyan-400"><UserGroupIcon className="h-7 w-7 text-cyan-700" /><h2 className="mt-4 font-bold text-slate-800">Daftar Undangan</h2><p className="mt-2 text-sm text-slate-500">Kelola nama, instansi, jabatan, kontak, dan email peserta.</p></Link><Link href={`/releases/${id}/qna`} className="rounded-2xl border border-slate-200 bg-white p-6 transition hover:border-violet-400"><ChatBubbleLeftRightIcon className="h-7 w-7 text-violet-700" /><h2 className="mt-4 font-bold text-slate-800">Sesi Q&A</h2><p className="mt-2 text-sm text-slate-500">Catat pertanyaan, susun saran AI, dan tetapkan jawaban final.</p></Link><Link href={`/releases/${id}/minutes`} className="rounded-2xl border border-slate-200 bg-white p-6 transition hover:border-blue-400"><ClockIcon className="h-7 w-7 text-blue-700" /><h2 className="mt-4 font-bold text-slate-800">Notulen Rilis</h2><p className="mt-2 text-sm text-slate-500">Gabungkan agenda, peserta, dan Q&A menjadi DOCX/PDF.</p></Link></section>
    </>}
  </div></AppShell>;
}
