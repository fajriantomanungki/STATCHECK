...
import { deleteBRS, getBRSList, getCurrentUser } from "@/services/api";
...
export default function BRSPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      Promise.all([getBRSList(token), getCurrentUser(token)])
        .then(([brsItems, currentUser]) => {
          setItems(brsItems);
          setUser(currentUser);
        })
  ...
  async function remove(item: BRS) {
    const token = getToken();
    if (!token) return;
    const confirmed = window.confirm(
      `Hapus BRS “${item.nama_brs}”?\n\nData, dokumen, hasil pemeriksaan, dan riwayat persetujuannya juga akan dihapus. Tindakan ini tidak dapat dibatalkan.`,
    );
    if (!confirmed) return;
    ...
    setMessage("");
    try {
      await deleteBRS(token, item.id);
      setItems((current) => current.filter((brs) => brs.id !== item.id));
      setMessage(`BRS “${item.nama_brs}” berhasil dihapus.`);
    } catch (err) {
      setError(err instanceof Error ? err.message : "BRS gagal dihapus.");
    } finally {
      setDeletingId(null);
    }
  }
