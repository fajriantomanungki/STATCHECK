  ...
  return <AppShell title="Detail Persetujuan">
    <div className="p-6 lg:p-10">
      ...
      {workflow && <>
        ...
        {revision ? <section className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-6"><h2 className="font-bold text-red-800">BRS dikembalikan untuk revisi</h2><p className="mt-2 text-sm leading-6 text-red-700">Perbaiki data atau dokumen berdasarkan catatan terakhir, lalu jalankan ulang STATCHECK. Setelah semua temuan ditindaklanjuti, BRS dapat dikirim kembali dari awal alur persetujuan.</p><Link href={`/brs/${id}/checking`} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-red-700 px-4 py-3 text-sm font-semibold text-white"><ArrowPathIcon className="h-5 w-5" />Perbaiki dan periksa ulang</Link></section> : workflow.current_status === "release_ready" ? <section className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-7 text-center"><CheckCircleIcon className="mx-auto h-12 w-12 text-emerald-600" /><h2 className="mt-3 text-xl font-bold text-emerald-900">BRS SIAP RILIS</h2><p className="mt-2 text-sm text-emerald-700">Seluruh tahap pemeriksaan dan persetujuan telah selesai.</p></section> : <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6"><h2 className="font-bold text-slate-800">Keputusan dan Catatan</h2><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={4} placeholder="Tambahkan catatan keputusan (wajib untuk revisi)..." className="mt-4 w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-cyan-500" /><div className="mt-4 flex flex-wrap gap-3">
