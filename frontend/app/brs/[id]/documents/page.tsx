...
const documentTypes: {
  ...
  {
    type: "bahan_publikasi",
    title: "BRS / Bahan Publikasi",
    description: "Naskah publikasi resmi BRS",
  },
  {
    type: "bahan_paparan",
    title: "Bahan Paparan",
    description: "Slide atau materi presentasi",
  },
  {
    type: "narasi_pimpinan",
    title: "Narasi Pimpinan",
    description: "Naskah yang dibacakan pimpinan",
  },
];
...
export default function BRSDocumentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!token) return;
    const [brsResult, documentResult] = await Promise.all([
      getBRS(token, id),
      getDocuments(token, id, true),
    ]);
    setBrs(brsResult);
  ...
  const activeByType = useMemo(() => {
    ...
    documents
      ...
      .forEach((item) => result.set(item.document_type, item));
    return result;
  }, [documents]);
  ...
  async function submitUpload(type: DocumentType) {
    ...
    try {
      ...
      setSelected(result);
      setMessage(
        `${file.name} berhasil diunggah sebagai versi ${result.version}.`,
      );
      await load();
    } catch (err) {
  ...
  async function showDetail(document: BRSDocument) {
    ...
    setError("");
    try {
      setSelected(await getDocument(token, document.id));
    } catch (err) {
      setError(
  ...
  async function handleDownload(document: BRSDocument) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Dokumen gagal diunduh.");
    }
  }
    ...
    try {
      ...
      setMessage(
        result.extraction_status === "completed"
          ? "Ekstraksi teks berhasil dijalankan ulang."
          : "Ekstraksi masih gagal. Periksa file sumber.",
      );
      await load();
    } catch (err) {
    ...
    <AppShell title="Dokumen BRS">
      <div className="p-6 lg:p-10">
        ...
        <div className="mt-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <div className="rounded-xl bg-[#102a43] px-5 py-3 text-sm text-white">
            <span className="text-slate-300">Kelengkapan</span>
            <strong className="ml-3 text-lg">{activeByType.size}/3</strong>
          </div>
        </div>
        ...
        <div className="mt-7 grid gap-5 xl:grid-cols-3">
          {documentTypes.map((config) => {
            ...
            const history = documents.filter(
              (item) =>
                item.document_type === config.type &&
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <DocumentArrowUpIcon className="h-7 w-7 shrink-0 text-cyan-700" />
                </div>
                  ...
                  {current ? (
                    <>
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <ExtractionBadge document={current} />
                      </div>
                      <p
                        className="mt-3 truncate font-semibold text-slate-700"
                        title={current.file_name}
                      >
                        {current.file_name}
                      </p>
                      <p className="mt-1 text-xs text-slate-400">
                        {formatSize(current.file_size)} • {current.page_count}{" "}
                        halaman/slide •{" "}
                        {current.extracted_char_count.toLocaleString("id-ID")}{" "}
                        karakter
                      </p>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <button
                          onClick={() => showDetail(current)}
                          className="inline-flex items-center gap-1 rounded-lg bg-cyan-50 px-3 py-2 text-xs font-semibold text-cyan-800"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg bg-slate-200 px-3 py-2 text-xs font-semibold text-slate-700"
                        >
                        ...
                        {current.extraction_status === "failed" && (
                          <button
                            onClick={() => handleReextract(current)}
                            className="inline-flex items-center gap-1 rounded-lg bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700"
                          >
                ...
                <label className="mt-4 block cursor-pointer rounded-xl border border-dashed border-slate-300 p-3 text-center text-sm text-slate-500 hover:border-cyan-500 hover:bg-cyan-50/40">
                  <input
                    type="file"
                    accept=".pdf,.pptx,.docx"
                    className="hidden"
                    onChange={(event) => chooseFile(config.type, event)}
                  />
                  {file ? (
                    ...
                    "Pilih PDF, PPTX, atau DOCX"
                  )}
                </label>
                <button
                  disabled={!file || busyType !== null}
                  onClick={() => submitUpload(config.type)}
                  className="mt-3 rounded-xl bg-[#102a43] px-4 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"
                >
                      ...
                      {history.map((item) => (
                        ...
                        >
                          <button
                            ...
                            className="min-w-0 truncate text-left font-semibold text-slate-600 hover:text-cyan-700"
                          >
                ...
                <>
                  <div className="flex flex-col justify-between gap-3 md:flex-row md:items-start">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-wider text-cyan-700">
                        Hasil Ekstraksi • Versi {selected.version}
                      </p>
                      <h2 className="mt-1 text-xl font-bold text-slate-800">
                        {selected.file_name}
                      </h2>
                      <p className="mt-1 text-sm text-slate-400">
                        Teks ini akan dibandingkan dengan dua dokumen lainnya
                        oleh mesin pemeriksaan Phase 8.
                      </p>
                    </div>
                    <button
                      onClick={() => setSelected(null)}
                      className="self-start rounded-lg bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-600"
                    >
                      Tutup
                    </button>
                  </div>
                  ...
                  <div className="mt-5 space-y-4">
                    {selected.contents.map((content) => (
                      ...
                      >
                        <h3 className="border-b border-slate-100 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-600">
                          {content.section_label}
                        </h3>
                        <pre className="max-h-96 overflow-auto whitespace-pre-wrap p-4 font-sans text-sm leading-6 text-slate-700">
                          {content.text_content ||
                            "Tidak ada teks yang dapat diekstrak pada bagian ini."}
                        </pre>
                      </article>
                    ))}
